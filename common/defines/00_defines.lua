...
NProduction = {	
    ...
    MIN_NAVAL_EQUIPMENT_CONVERSION_RESOURCE_COST_FACTOR = 1,    -- Minimum fraction of an equipment type's base strategic resource cost to use when converting a naval equipment, such as through ship refitting.  
},
...
NMilitary = {	
    ...
    LAND_COMBAT_FORT_DAMAGE_CHANCE = 50,        -- chance to get a hit to damage on forts. (out of 100)    
    ...
    MULTIPLE_COMBATS_PENALTY = -0.3,               -- defender penalty if attacked from multiple directions         
	...
	REINFORCEMENT_REQUEST_MAX_WAITING_DAYS = 7,   -- Every X days the equipment will be sent, regardless if still didn't produced all that has been requested.   
    REINFORCEMENT_REQUEST_DAYS_FREQUENCY = 3,       -- How many days must pass until we may give another reinforcement request        
    EXPERIENCE_COMBAT_FACTOR = 0.05,            -- askerler cok xp kazanirsa degisecek
	PARACHUTE_COMPLETE_ORG = 0.2,                   -- Organisation value (in %) after unit being dropped, regardless if failed, disrupted, or successful.  
    ...
    PLAN_PROVINCE_CAPITAL_IMPORTANCE_AREA = 2.0,    -- Used when calculating the balue of defense area in the battle plan system         
	PLAN_PROVINCE_MEDIUM_VP_IMPORTANCE_FRONT = 2.0, -- Used when calculating the calue of fronts in the battle plan system
    PLAN_PROVINCE_HIGH_VP_IMPORTANCE_FRONT = 2.0,  -- Used when calculating the calue of fronts in the battle plan system         
	PLAN_PORVINCE_PORT_BASE_IMPORTANCE = 2.0,        -- Added importance for area defense province with a port     
