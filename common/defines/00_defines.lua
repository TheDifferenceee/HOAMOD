...
NPolitics = {	
	...
	NAVY_LEADER_MAX_COST = 100,                -- max cost BEFORE modifiers    (100den düşürebilirim general icin sonra bak)
},
...
NMilitary = {	
    ...
    LAND_COMBAT_FORT_DAMAGE_CHANCE = 50,        -- chance to get a hit to damage on forts. (out of 100)      (vanillası 5)
	CHANCE_TO_AVOID_HIT_AT_NO_DEF = 60,               -- chance to avoid hit if no defences left.     (vanilla, değiştirebilirim sonradan)
	...
	BASE_FORT_PENALTY = -0.1,                        -- fort penalty              değişecek
    MULTIPLE_COMBATS_PENALTY = -0.3,               -- defender penalty if attacked from multiple directions           değişecek
    DIG_IN_FACTOR = 0.02,                           -- bonus factor for each dug-in level                        (kodu ararsam burada bulayım, entrench.) (vanilla)
...
NAir = {	
    ...
    COMBAT_DAMAGE_SCALE_CARRIER = 5,                    -- same as above but used inside naval combat for carrier battles                uçakların aldigi hasar
    ...
    NAVAL_STRIKE_CARRIER_MULTIPLIER = 10.0,              -- damage bonus when planes are in naval combat where their carrier is present (and can thus sortie faster and more effectively)     elimde kalsın DONANMA
...
NNavy = {	
	...
	MAX_ANTI_AIR_REDUCTION_EFFECT_ON_INCOMING_AIR_DAMAGE             = 0.75,    -- damage reduction for incoming air attacks is clamped to this value at maximum.         (vanilla değer)        üstteki yazının devamı
	DEPTH_CHARGES_HIT_PROFILE                                         = 100.0,    -- hit profile for depth charges                    zazoda 1, tam emin degiliz ama dept charge güclü zazoda???
	...
	NAVAL_COMBAT_AIR_LOW_AA_TARGET_SCORE = 5,                           -- how much score factor from low AA guns (scales between 0->this number)           ZAZODA HEPSİ 1, ARADIĞIM DEFINE BU DONANMA ICIN
},
