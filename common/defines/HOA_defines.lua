...
NDefines.NMilitary.LAND_AIR_COMBAT_MAX_PLANES_PER_ENEMY_WIDTH = 3 -- how many CAS/TAC can enter a combat depending on enemy width there --3 
...
NDefines.NMilitary.DIG_IN_FACTOR = 0.02                           -- bonus factor for each dug-in level --0.02 yani vanilla degeri sonradan bakabilirim                      
NDefines.NMilitary.ENEMY_AIR_SUPERIORITY_IMPACT = -0.10          -- effect on defense due to enemy air superiorty -- -0.35
NDefines.NMilitary.ENEMY_AIR_SUPERIORITY_DEFENSE = 1           -- more AA attack will approach this amount of help (diminishing returns) --0.70
NDefines.NMilitary.ENEMY_AIR_SUPERIORITY_DEFENSE_STEEPNESS = 120 -- how quickly defense approaches the max impact diminishing returns curve --112
NDefines.NMilitary.ENEMY_AIR_SUPERIORITY_SPEED_IMPACT = -0.1     -- effect on speed due to enemy air superiority -- -0.3
NDefines.NMilitary.ANTI_AIR_TARGETTING_TO_CHANCE = 0.07            -- Balancing value to determine the chance of ground AA hitting an attacking airplane, affecting both the effective average damage done by AA to airplanes, and the reduction of damage done by airplanes due to AA support --0.07 yani vanilla degeri sonradan bakabilirim           
...
NDefines.NAir.AIR_DEPLOYMENT_DAYS = 1                            -- Days to deploy one air wing --2 
...
NDefines.NNavy.NAVAL_MINES_IN_REGION_MAX = 0.0 --1000.0
NDefines.NNavy.NAVAL_MINES_PLANTING_SPEED_MULT = 0.00000 --0.01
NDefines.NNavy.MAX_POSITIONING_PENALTY_FROM_HIGHER_SHIP_RATIO = 0.75  -- maximum penalty to get from larger fleets --0.75 yani vanilla deger   
...
NDefines.NNavy.NAVAL_COMBAT_AIR_SUB_TARGET_SCORE = 10                             -- scoring for target picking for planes inside naval combat, one define per ship typ --10 yani vanilla deger
NDefines.NNavy.NAVAL_COMBAT_AIR_CAPITAL_TARGET_SCORE = 50 --50 yani vanilla deger
NDefines.NNavy.NAVAL_COMBAT_AIR_CARRIER_TARGET_SCORE = 200 --200 yani vanilla deger
NDefines.NNavy.NAVAL_COMBAT_AIR_CONVOY_TARGET_SCORE = 1.0 --1.0 yani vanilla deger
NDefines.NNavy.NAVAL_COMBAT_AIR_STRENGTH_TARGET_SCORE = 5                         -- how much score factor from low health (scales between 0->this number) --5 yani vanilla deger
NDefines.NNavy.NAVAL_COMBAT_AIR_LOW_AA_TARGET_SCORE = 5                           -- how much score factor from low AA guns (scales between 0->this number) --5 yani vanilla deger
